...
export default function Checkout() {
  ...
  const canPlace =
    !submitting && subtotal > 0 && name.trim().length > 1 && email.trim().length > 3;
            ...
            {orderId ? (
              <div className="mt-8 grid gap-6 lg:grid-cols-[minmax(0,1fr)_320px]">
                  ...
                  {savedPaymentMethod === "bank_transfer" ? (
                    <div className="mt-6 rounded-xl2 border border-line bg-white p-5">
                      ...
                      <div className="mt-4 grid gap-2 text-sm text-muted">
                        <div>
                          <span className="font-extrabold text-ink">Amount due:</span> {formatGBP(total)}
                        </div>
                        <div>
                      ...
                      <p className="mt-4 text-sm text-muted">
                        Please use your order number as the payment reference. Your order will be
                        processed once payment is received.
                      </p>
                    </div>
                  ) : (
                    <div className="mt-6 rounded-xl2 border border-line bg-white p-5">
                      ...
                      <div className="mt-4 grid gap-4 text-sm text-muted">
                        <div>
                          <div className="font-extrabold text-ink">Amount due</div>
                          <div>{formatGBP(total)}</div>
                        </div>
                        <div>
                          <div className="font-extrabold text-ink">Bitcoin (BTC)</div>
                          <div className="break-all">{PAYMENT_DETAILS.crypto.btc}</div>
                        </div>
                        <div>
                          <div className="font-extrabold text-ink">Ethereum (ETH)</div>
                          <div className="break-all">{PAYMENT_DETAILS.crypto.eth}</div>
                        </div>
                        <div>
                          <div className="font-extrabold text-ink">USDT (ERC20 only)</div>
                          <div className="break-all">{PAYMENT_DETAILS.crypto.usdtErc20}</div>
                        </div>
                        <div>
                          <span className="font-extrabold text-ink">Reference:</span> {orderId}
                        </div>
                      ...
                      <p className="mt-4 text-sm text-muted">
                        Please use your order number as your payment reference. Only send USDT on the
                        ERC20 network.
                      </p>
                    </div>
                  )}
                ...
                <div className="rounded-xl2 border border-line bg-panel p-6">
                  ...
                  <div className="mt-6">
                    <div className="text-sm font-extrabold text-ink">Items ordered</div>
                    ...
                    <div className="mt-4 grid gap-3">
                      ...
                      <label className="flex cursor-pointer items-start gap-3 rounded-xl2 border border-line bg-white p-4">
                        ...
                        <div>
                          ...
                          <div className="mt-1 text-sm text-muted">
                            Pay with BTC, ETH or USDT (ERC20 only) after placing your order.
                          </div>
                        </div>
                      </label>
                    </div>
