...
<<<<<<< HEAD
      ...
      .filter(Boolean) as Array<StoredOrderItem & { image: string; pack: string; subtitle: string; coa?: string }>;
  }, [cart, productMap]);

  const subtotal = useMemo(() => orderItems.reduce((sum, item) => sum + item.priceGBP * item.qty, 0), [orderItems]);
=======
        };
      })
...
>>>>>>> da4884999aac5e65a9be772f3a18c2b688e4ec9f
    ...
    try {
      const payload = {
        ...
        paymentMethod,
<<<<<<< HEAD
        items: orderItems.map(({ id, name: itemName, qty, priceGBP }) => ({ id, name: itemName, qty, priceGBP })),
=======
      ...
      <main className="py-10 lg:py-12">
        <Container>
          <div className="mx-auto max-w-6xl rounded-[1.75rem] border border-line bg-white p-6 shadow-soft lg:p-10">
            ...
            <h1 className="mt-4 text-3xl font-extrabold tracking-tight text-ink">Bank transfer or cryptocurrency checkout.</h1>
            <p className="mt-2 max-w-3xl text-sm text-muted">
              Use this page for bank transfer or cryptocurrency orders. For card payment, return to cart and use the secure Stripe checkout button.
            </p>
            ...
            {orderId ? (
              <div className="mt-8 grid gap-6 lg:grid-cols-[minmax(0,1fr)_340px]">
                <div className="rounded-xl2 border border-line bg-panel p-6">
                  ...
                  <div className="mt-2 text-3xl font-extrabold tracking-tight text-accent">{orderId}</div>
                  <div className="mt-3 text-sm text-muted">
                    Payment method: <span className="font-extrabold text-ink">{savedPaymentMethod === "bank_transfer" ? "Bank Transfer" : "Cryptocurrency"}</span>
=======
...
<<<<<<< HEAD
                      <div className="text-lg font-extrabold text-ink">Bank transfer instructions</div>
                      <div className="mt-4 grid gap-2 text-sm text-muted">
                        <div><span className="font-extrabold text-ink">Amount due:</span> {formatGBP(total)}</div>
                        <div><span className="font-extrabold text-ink">Account name:</span> {PAYMENT_DETAILS.bank.accountName}</div>
                        <div><span className="font-extrabold text-ink">Sort code:</span> {PAYMENT_DETAILS.bank.sortCode}</div>
                        <div><span className="font-extrabold text-ink">Account number:</span> {PAYMENT_DETAILS.bank.accountNumber}</div>
                        <div><span className="font-extrabold text-ink">Reference:</span> {orderId}</div>
                      </div>
                      <p className="mt-4 text-sm text-muted">Please use your order number as the payment reference. Your order will be processed once payment is received.</p>
                    </div>
                  ) : (
                    <div className="mt-6 rounded-xl2 border border-line bg-white p-5">
                      <div className="text-lg font-extrabold text-ink">Cryptocurrency instructions</div>
                      <div className="mt-3 text-sm text-muted">Please send <span className="font-extrabold text-ink">{formatGBP(total)}</span> using one of the options below.</div>
                      <div className="mt-3 text-sm text-muted">Use your order number as the payment reference: <span className="font-extrabold text-ink">{orderId}</span></div>
                      <div className="mt-5 space-y-4">
                        ...
                        <CryptoAddressCard label="USDT (ERC20 only)" address={PAYMENT_DETAILS.crypto.usdtErc20} note="Only send USDT on the ERC20 network." />
                      </div>
                      <p className="mt-4 text-sm text-muted">Your order will be processed after payment is received and confirmed.</p>
=======
...
<<<<<<< HEAD
                    ...
                    <Link href="/shop" className="rounded-xl2 border border-line bg-white px-5 py-3 text-sm font-extrabold text-ink shadow-soft hover:bg-panel">Continue shopping</Link>
=======
                    <Link
                      href="/order-status"
                ...
                <div className="space-y-6">
                  <div className="rounded-xl2 border border-line bg-panel p-5">
                    ...
                    <p className="mt-3 text-sm leading-7 text-muted">
                      This page is for customers choosing bank transfer or cryptocurrency. If you want to pay by card, return to your <Link href="/cart" className="font-semibold text-ink hover:text-accent">cart</Link> and use the secure Stripe checkout button.
                    </p>
                    <div className="mt-4 flex flex-wrap gap-3">
                      <Link href="/cart" className="rounded-xl2 bg-accent px-5 py-3 text-sm font-extrabold text-white shadow-soft hover:bg-accent/90">Return to cart</Link>
                      <Link href="/quality-assurance" className="rounded-xl2 border border-line bg-white px-5 py-3 text-sm font-extrabold text-ink shadow-soft hover:bg-panel">Review documentation</Link>
=======
...
<<<<<<< HEAD
                    ...
                    <input type="text" value={name} onChange={(e) => setName(e.target.value)} className="mt-3 w-full rounded-xl2 border border-line bg-panel px-5 py-4 text-base text-ink outline-none transition focus:border-accent" placeholder="Enter your full name" />
                  </div>

                  <div>
                    <label className="block text-sm font-extrabold text-ink">Email</label>
                    <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} className="mt-3 w-full rounded-xl2 border border-line bg-panel px-5 py-4 text-base text-ink outline-none transition focus:border-accent" placeholder="Enter your email" />
                  </div>
                  ...
                  <div className="rounded-xl2 border border-line bg-panel p-5">
                    ...
                    <div className="mt-4 grid gap-3">
                      <label className="flex cursor-pointer items-start gap-3 rounded-xl2 border border-line bg-white p-4">
                        <input type="radio" name="paymentMethod" checked={paymentMethod === "bank_transfer"} onChange={() => setPaymentMethod("bank_transfer")} className="mt-1" />
                        <div>
                          <div className="text-sm font-extrabold text-ink">Bank transfer</div>
                          <div className="mt-1 text-sm text-muted">Place your order now and pay by UK bank transfer.</div>
=======
...
>>>>>>> da4884999aac5e65a9be772f3a18c2b688e4ec9f
                      ...
                      <label className="flex cursor-pointer items-start gap-3 rounded-xl2 border border-line bg-white p-4">
<<<<<<< HEAD
                        <input type="radio" name="paymentMethod" checked={paymentMethod === "crypto"} onChange={() => setPaymentMethod("crypto")} className="mt-1" />
                        <div>
                          <div className="text-sm font-extrabold text-ink">Cryptocurrency</div>
                          <div className="mt-1 text-sm text-muted">Place your order now and pay with BTC, ETH or USDT (ERC20 only).</div>
=======
                        <input
                          type="radio"
                          name="paymentMethod"
...
<<<<<<< HEAD
                    ...
                    {orderItems.length === 0 ? (
                      ...
                      <div className="mt-4 space-y-4">
                        {orderItems.map((item) => (
                          <div key={item.id} className="grid gap-4 rounded-xl2 border border-line bg-white p-4 sm:grid-cols-[88px_minmax(0,1fr)_120px] sm:items-center">
                            ...
                            <div className="text-left text-sm font-extrabold text-ink sm:text-right">{formatGBP(item.priceGBP * item.qty)}</div>
=======
                    <div className="text-lg font-extrabold text-ink">
                      Items ordered
                    </div>
...
<<<<<<< HEAD
                <aside className="h-fit rounded-xl2 border border-line bg-panel p-6 lg:sticky lg:top-28">
                  ...
                  <div className="mt-5 grid gap-2 text-sm text-muted">
                    <div className="rounded-xl2 border border-line bg-white px-4 py-3">Order is saved before payment instructions are shown</div>
                    <div className="rounded-xl2 border border-line bg-white px-4 py-3">Card payment remains available via Stripe in cart</div>
                    <div className="rounded-xl2 border border-line bg-white px-4 py-3">Quality page available for certificate review</div>
                  </div>

                  <button type="button" onClick={placeOrder} disabled={!canPlace} className={"mt-6 w-full rounded-xl2 px-4 py-3 text-sm font-extrabold text-white shadow-soft transition " + (canPlace ? "bg-accent hover:bg-accent/90" : "cursor-not-allowed bg-accent/40")}>
...
<<<<<<< HEAD
              <Link href="/cart" className="font-extrabold text-ink/80 hover:text-ink">← Back to cart</Link>
